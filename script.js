...
function buildTree(list, startKey) {
  const keyMap = { diretoria:"diretoria", gerencia:"gerenciaRegional", agencia:"agencia", gGestao:"gerenteGestao", gerente:"gerente", familia:"familia", prodsub:"prodOrSub", produto:"prodOrSub", detalheVenda:"__detalhe__" };
  const NEXT   = { diretoria:"gerencia",  gerencia:"agencia",         agencia:"gGestao", gGestao:"gerente",       gerente:"prodsub", familia:"contrato",   prodsub:"detalheVenda", detalheVenda:"contrato" };
  ...
  function buildLevel(arr, levelKey, level){
    ...
    if (levelKey === "detalheVenda") {
      const map = new Map();
      arr.forEach(r => {
        const canal = r.canalVenda || "Canal não informado";
        const tipo = r.tipoVenda || "Tipo não informado";
        const modal = r.modalidadePagamento || (r.subproduto || "Modalidade não informada");
        const gerente = r.gerente || "—";
        const key = `${canal}|${tipo}|${modal}|${gerente}`;
        const bucket = map.get(key) || [];
        bucket.push(r);
        map.set(key, bucket);
      });
      return [...map.entries()].map(([comboKey, subset]) => {
        const [canal, tipo, modal, gerente] = comboKey.split("|");
        const a = agg(subset);
        const next = NEXT[levelKey];
        return {
          type:"grupo",
          level,
          label: gerente || canal || tipo,
          realizado: a.realizado,
          meta: a.meta,
          qtd: a.qtd,
          ating: a.ating,
          data: a.data,
          detail: { canal, tipo, modalidade: modal, gerente },
