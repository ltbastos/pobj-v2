...
function determineCampaignDisplayLevel(filters = getFilterValues()) {
  if (filters.gerente && filters.gerente !== "Todos") {
    return { level: "carteira", meta: CAMPAIGN_LEVEL_META.carteira };
  }
...
function renderCampanhasView(){
  ...
  if (noteEl) {
    const visibleUnits = summaryInfo.total;
    const base = sprint.note || "";
    const suffix = visibleUnits ? ` Exibindo ${fmtINT.format(visibleUnits)} carteiras filtradas.` : " Nenhuma unidade encontrada para o filtro atual.";
...
function renderRanking(){
  ...
  dataClamped.forEach((r,idx)=>{
    ...
    tr.innerHTML = `
      ...
      <td class="unit-col rk-name" title="${escapeHTML(r.unidade || nome)}">${nomeSafe}</td>
